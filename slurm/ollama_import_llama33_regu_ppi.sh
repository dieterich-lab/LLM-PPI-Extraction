#!/bin/bash
#
# Import the fine-tuned Llama 3.3 70B (RegulaTome PPI) model into Ollama.
#
# Uses the Unsloth-generated Modelfile from the GGUF directory and
# registers it as 'llama3.3:70b-regulatome-ppi' in Ollama.
#
# Prerequisites:
#   - Finetuning must have completed (job 678484 or similar)
#   - Ollama must be installed on the node
#
# Usage:
#   sbatch slurm/ollama_import_llama33_regu_ppi.sh

#SBATCH --job-name=ollama_import
#SBATCH --output=../outputs/slurm/ollama_import_llama33_regu_ppi_%j.txt
...
MODEL_DIR="${OUTPUT_ROOT}/finetunedmodels/unsloth/Llama-3.3-70B-Instruct-bnb-4bit_regulatome_ppi_GGUF"
MODEL_NAME="llama3.3:70b-regulatome-ppi"
...
mkdir -p "$LOG_DIR"
OLLAMA_LOG="${LOG_DIR}/ollama_import_33_regu_${SLURM_JOB_ID:-local}.log"
...
echo "Importing model from $MODEL_DIR (this will take several minutes for 70B)..."
