#!/bin/bash
#SBATCH --job-name=bench70b_nomax
#SBATCH --output=/beegfs/prj/LINDA_LLM/outputs/slurm/bench_ppi_nomax_%j.log
...
OLLAMA_LOG="${SLURM_LOG_DIR}/ollama_bench_ppi_nomax_${SLURM_JOB_ID:-local}.log"
...
# Intentionally do NOT export MAX_CHARS to test default/internal truncation behavior.
...
echo "Starting no-MAX_CHARS benchmark"
echo "Model: $MODEL"
echo "N_DOCS: $N_DOCS"
...
echo "MAX_CHARS env: ${MAX_CHARS-<unset>}"
...
python -u extract.py \
  ...
  --untildoc "$N_DOCS" \
  --force_new \
  --ext "bench_nomax_${MODEL}_${SLURM_JOB_ID:-local}"
